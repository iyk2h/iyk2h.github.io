...
enum(열거형)은 Enum을 '열거형' 또는 Enumeration 또는 상수집합 이라고도 부른다.

서로 관련된 상수를 편리하게 선언하기 위한 것으로 상수를 여러 개 정의할 때 사용한다. 
...
상수를 정의하는 방법을 개선하다보니 열거형이 적합하기 때문이다. 추가로, 열거형을 사용하면 코드의 가독성을 높이고 논리적인 오류를 줄일 수 있다.

자바 1.5버전 이전에 어떻게 상수를 정의했는지 알아보면서 왜 생겼는지 알아보자.
...
enum은 각 클래스를 인스턴스화 시킨 것이기 때문에 오버라이딩이 가능하다.
